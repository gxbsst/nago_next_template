...
export async function fetchMenu() {
  ...
  try {
    const data = await client.GetAppMenu({id: "app"});

    return data?.menu;
  } catch (err) {
    console.error(err || 'Failed to fetch menu!!!');
  }
}

export async function fetchPageBySlug(slug: string = "http://nadi_next.home/") {
  try {
    const client = getClientWithSdk();
    const data = await client.GetHomePage({uri: slug});
    if (!data) {
      new Error('Post not found!!!');
    }
    return data?.pageBy;
  } catch (err) {
    console.error(err || 'Failed to fetch post data!!!');
  }
}
...
export async function fetchPostBySlug(slug:string) {
  try {
    const client = getClientWithSdk();
    const data = await client.GetPostBySlug({slug});
    if (!data) {
      new Error('Post not found!!!');
    }
    return data?.post;
...
export async function fetchPostsBy(where: RootQueryToPostConnectionWhereArgs, limit = 3) {
  try {
    const client = getClientWithSdk();
    const data = await client.GetPostsBy({where, limit});
    if (!data) {
      new Error('Post not found!!!');
    }
    return data?.posts;
